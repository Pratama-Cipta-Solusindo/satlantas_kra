	...
	<div class="card-body">
			...
			if ($mod == 'filter' AND $str_tanggal1 != '' AND $str_tanggal2 != '') {
				...
				$sql = "SELECT COUNT(r.id_responden) as data, r.jk 
							FROM responden r
								INNER JOIN polling p ON r.signature=p.signature
							WHERE p.satuan='$SatuanID' 
								AND p.waktu BETWEEN '". $str_tanggal1 ." 00:00:00' AND '". $str_tanggal2 ." 23:59:59' 
							GROUP BY r.jk";
				echo $sql;
				$exe = _query($sql);
				$jmlResponden = 0;
				...
				$sql = "SELECT COUNT(r.id_responden) as data, r.agama 
							FROM responden r
								INNER JOIN polling p ON r.signature=p.signature
							WHERE p.satuan='$SatuanID' 
								AND (LEFT(p.waktu, 10) BETWEEN '$str_tanggal1' AND '$str_tanggal2') 
							GROUP BY r.agama";
				#echo $sql;
				$exe = _query($sql);
				while ($row = _object($exe)) {
				...
				#- nilai polling 
				$sql = "SELECT COUNT(r.id_responden) as data, p.nilai 
							FROM responden r
								INNER JOIN polling p ON r.signature=p.signature
							WHERE p.satuan='$SatuanID' 
								AND (LEFT(p.waktu, 10) BETWEEN '$str_tanggal1' AND '$str_tanggal2') 
